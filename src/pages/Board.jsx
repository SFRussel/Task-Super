...
const Board = () => {
  ...
  const handleSaveList = async (listTitle) => {
    console.log('Saving list with title:', listTitle, 'Editing:', !!editingList);
    
    try {
      if (editingList) {
        // Rename existing list
        console.log('Renaming list:', editingList.id, 'from', editingList.title, 'to', listTitle);
        ...
        if (currentBoard) {
          const result = await updateBoard(currentBoard.id, { lists: updatedLists });
          console.log('Board updated after rename:', result);
        }
        ...
        if (currentBoard) {
          const result = await updateBoard(currentBoard.id, { lists: updatedLists });
          console.log('Board updated after list creation:', result);
        }
  ...
  const handleDeleteList = async (listId) => {
    ...
    console.log('Tasks in list:', tasksInList.length);
  ...
  const confirmDeleteList = async () => {
    ...
    console.log('Confirming list deletion:', listId, 'with', taskCount, 'tasks');
    
    try {
      // Delete all tasks in the list
      if (taskCount > 0) {
        const tasksInList = tasks.filter(t => t.listId === listId);
        console.log('Deleting tasks:', tasksInList.map(t => t.id));
      ...
      setLists(updatedLists);
      console.log('List deleted, remaining lists:', updatedLists.length);
  ...
  const handleRenameList = (listId) => {
    console.log('Rename list clicked:', listId);
    ...
    console.log('Opening rename modal for list:', list.title);
    setEditingList(list);
    setIsListModalOpen(true);
  };
